...
from pyoscal.core import *  # noqa: F401,F403 # necessary for exec statements
...
class OSCAL_XML(OSCAL_IO):
                ...
                if param is not None:
                    try:
                        if isinstance(param, str):
                            root.attrib[parameter] = param
                        else:
                            root.attrib[param.use_name] = param.prose
                    except (AttributeError, TypeError, ValueError) as e:
                        print("Failed creating {}".format(obj.__class__))
                        print("  Error: {}".format(e))
                        print("  XMLRoot: {}".format(root.tag))
                        print("  Obj: {}".format(obj.__dict__))
                        print("  Parameter: {}".format(parameter))
                        print("  Param: {}".format(param))
                        print("  Param_class: {}".format(parameter.__class__))
    ...
    def export(self, obj):
        ...
        result.attrib['xmlns'] = self.ns['oscal']
